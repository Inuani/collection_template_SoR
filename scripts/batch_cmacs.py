...
##########################################################################
# BATCH CMAC PROCESSING WITH DFX
##########################################################################
...
def main():
    if len(sys.argv) not in [5, 6]:
        print("Usage: python3 batch_cmacs.py <cmacs_file> <canister_name> <page_path> <uid> [--ic]")
        sys.exit(1)
    ...
    ic_mode = False
    if len(sys.argv) == 6 and sys.argv[5] == "--ic":
        ic_mode = True
    ...
    print(f"Processing {len(all_cmacs)} CMACs in {total_batches} batches for UID: {uid}")
    ...
    for i, batch in enumerate(batches, 1):
        ...
        cmd = f'dfx canister call {ic_flag}{canister_name} append_route_cmacs \'("{page_path}", "{uid}", {candid_data})\''
